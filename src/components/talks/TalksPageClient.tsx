...
import { Conference, ResearchArea } from '@/lib/types';
...
function ConferenceTypeBadge({ type }: { type: string }) {
  const styles: Record<string, string> = {
    ...
    testimony: 'bg-red-500/20 text-red-400',
...
function ConferenceItem({ conference }: { conference: Conference }) {
  const locationString = conference.location.virtual
    ? 'Virtual'
    : `${conference.location.city || ''}${conference.location.state ? `, ${conference.location.state}` : ''}${conference.location.city || conference.location.state ? ', ' : ''}${conference.location.country}`;
    ...
    <div className="card card-hover">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          {(conference.slidesUrl || conference.paperUrl || conference.videoUrl || conference.url) && (
...
export function TalksPageClient({ allConferences, researchAreas }: TalksPageClientProps) {
  const { filteredItems, counts } = useTagFilter(allConferences, researchAreas);

  // Group filtered conferences by year
    ...
    <div className="container-main py-12">
      <header className="mb-8">
        <h1 className="text-4xl font-bold mb-4">
          <span className="text-accent-primary">&gt;</span> Talks
        </h1>
        <p className="text-text-secondary max-w-2xl mb-6">
          Conference presentations, invited talks, testimony, and panel discussions on digital politics, misinformation, and political behaviour.
        </p>
        <TagFilter
          areas={researchAreas}
          counts={counts}
      ...
      <div className="relative">
        ...
        {years.length > 0 ? (
          ...
          <p className="text-text-tertiary">No talks found for this topic.</p>
        )}
      </div>
