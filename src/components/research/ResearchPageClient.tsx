      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between gap-2 mb-2">
            <span className={`badge ${pub.category === 'refereed' ? 'badge-refereed' : pub.category === 'conference' ? 'badge-refereed' : 'badge-report'}`}>
              {pub.category === 'refereed' ? 'Peer-Reviewed' : pub.category === 'conference' ? 'Conference' : pub.category === 'report' ? 'Report' : pub.category === 'dissertation' ? 'Dissertation' : 'Working Paper'}
            </span>
...
export function ResearchPageClient({ allPublications, researchAreas }: ResearchPageClientProps) {
  const { filteredItems, counts } = useTagFilter(allPublications, researchAreas);

  // Group filtered publications by year
  const publicationsByYear = useMemo(() => {
    return filteredItems.reduce((acc, pub) => {
      if (!acc[pub.year]) {
        acc[pub.year] = [];
      }
  ...
  return (
    <div className="container-main py-12">
      ...
      {years.length > 0 ? (
        ...
        <p className="text-text-tertiary">No publications found for this topic.</p>
      )}
