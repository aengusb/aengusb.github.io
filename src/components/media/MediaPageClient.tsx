...
import { MediaAppearance, ResearchArea } from '@/lib/types';
import { TagFilter, useTagFilter } from '@/components/ui/TagFilter';
...
interface MediaPageClientProps {
  allMedia: MediaAppearance[];
  featuredMedia: MediaAppearance[];
  researchAreas: ResearchArea[];
}

export function MediaPageClient({ allMedia, featuredMedia, researchAreas }: MediaPageClientProps) {
  const { selectedArea, filteredItems, counts } = useTagFilter(allMedia, researchAreas);

  // Group filtered media by year
  ...
  return (
    <div className="container-main py-12">
      ...
      {/* Featured Section */}
      {filteredFeatured.length > 0 && !selectedArea && (
        <section className="mb-16">
          <h2 className="section-title">Featured</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredFeatured.slice(0, 4).map((media) => (
      ...
      <section>
        <h2 className="section-title">
          {selectedArea ? `${researchAreas.find(a => a.id === selectedArea)?.label} Appearances` : 'All Appearances'}
          <span className="ml-2 text-sm font-normal text-text-tertiary">({filteredItems.length})</span>
        </h2>
        {years.length > 0 ? (
          ...
          <p className="text-text-tertiary">No media appearances found for this topic.</p>
        )}
      </section>
