...
export default function Slide({
  ...
  return (
    <div className={`${baseStyles} ${variantStyles[variant]} ${className}`}>
      ...
      {/* Main content */}
      <div className={`flex-1 w-full flex flex-col ${needsHeader ? 'p-12 pt-10' : 'p-14'} ${variant === 'section' || variant === 'title' || variant === 'thank-you' ? 'justify-center items-center text-center' : ''}`}>
        {children}
      </div>

      {/* Footer */}
      {showFooter && (
        <div className="absolute bottom-0 left-0 right-0 flex justify-between items-center px-12 py-4 border-t border-[var(--border-color)] bg-[var(--bg-primary)]/90">
          <span className="text-lg text-[var(--text-tertiary)] font-medium tracking-wide">
            {footerText}
          </span>
          {slideNumber && (
  ...
  return (
    <Slide variant="title" showFooter={false}>
      <div className="flex flex-col items-center max-w-[80%]">
        ...
        {category && (
          <p className="text-[var(--accent-primary)] text-2xl mb-6 tracking-wider font-medium animate-fade-in">
            <span className="opacity-60">&gt;</span> {category}
          </p>
        )}

        {/* Title */}
        <h1 className="text-7xl md:text-9xl font-bold text-[var(--accent-primary)] tracking-tight leading-tight mb-6 animate-slide-up">
          {title}
        </h1>

        {subtitle && (
          <p className="text-[var(--text-secondary)] text-3xl md:text-4xl mt-2 animate-fade-in stagger-2">
            {subtitle}
          </p>
        )}
        ...
        {/* Date / Author line */}
        <div className="flex items-center gap-4 text-[var(--text-tertiary)] text-xl animate-fade-in stagger-4">
          {date && <span>{date}</span>}
...
export function ContentSlide({ category, title, children, slideNumber, footerText }: ContentSlideProps) {
  return (
    <Slide variant="content" slideNumber={slideNumber} footerText={footerText}>
      ...
      {/* Content */}
      <div className="flex-1 w-full text-3xl text-[var(--text-primary)]">
        {children}
      </div>
    </Slide>
  );
    ...
    <Slide variant="two-column" slideNumber={slideNumber} footerText={footerText}>
      ...
      <div className="flex-1 w-full grid grid-cols-2 gap-10">
        <div className="flex flex-col">
          {questionTitle && (
            <h3 className="text-4xl font-bold text-[var(--accent-tertiary)] mb-6">
              {questionTitle}
            </h3>
          )}
          <div className="text-3xl text-[var(--text-primary)]">{left}</div>
        </div>
...
export function SectionSlide({ title, subtitle }: SectionSlideProps) {
  return (
    <Slide variant="section" showFooter={false}>
      <h2 className="text-7xl md:text-8xl font-bold text-[var(--accent-primary)] mb-4 animate-slide-up">
        {title}
      </h2>
      {subtitle && (
        <p className="text-3xl md:text-4xl text-[var(--text-secondary)] animate-fade-in stagger-2">
          {subtitle}
        </p>
      )}
    </Slide>
  ...
  return (
    <Slide variant="thank-you" showFooter={false}>
      <div className="flex items-center gap-16">
        ...
        <div>
          <h2 className="text-7xl font-bold text-[var(--accent-primary)] mb-6 animate-slide-up">
            {title}
          </h2>
          <div className="accent-line w-32 mb-6 animate-fade-in stagger-2" />
          {contacts?.map((contact, i) => (
            <p key={i} className="text-3xl text-[var(--text-secondary)] mb-2 animate-fade-in" style={{ animationDelay: `${0.3 + i * 0.1}s` }}>
...
export function BulletList({ items }: BulletListProps) {
  return (
    <ul className="terminal-bullets space-y-4 list-none">
      {items.map((item, index) => (
        <li key={index} className="text-3xl md:text-4xl leading-relaxed animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
          {item}
        </li>
      ))}
    </ul>
...
export function HighlightBox({ children, variant = 'teal' }: HighlightBoxProps) {
  ...
  return (
    <div className={`p-6 rounded-lg border-l-4 ${colors[variant]} bg-[var(--bg-secondary)] my-4 text-2xl`}>
      {children}
    </div>
  );
}
...
export function Stat({ value, label, color = 'teal' }: StatProps) {
  ...
  return (
    <div className="text-center p-4">
      <div className={`text-6xl md:text-7xl font-bold ${colors[color]} mb-2 animate-count-up`}>
        {value}
      </div>
      <div className="text-lg md:text-xl text-[var(--text-secondary)] uppercase tracking-wide">
        {label}
      </div>
    </div>
  );
...
export function Citation({ refs }: CitationProps) {
  return (
    <div className="absolute bottom-16 right-12 text-right z-10">
      {refs.map((ref, i) => (
        <p key={i} className="text-base text-[var(--text-tertiary)] opacity-60 leading-relaxed whitespace-nowrap">
          {ref}
        </p>
      ))}
    </div>
...
export function Figure({ src, alt, caption }: FigureProps) {
  return (
    <figure className="flex flex-col items-center">
      ...
      {caption && (
        <figcaption className="text-lg text-[var(--text-tertiary)] mt-3 text-center">
          {caption}
        </figcaption>
      )}
    </figure>
